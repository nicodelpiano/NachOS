...
class List {
  public:
    ...
    void Remove(Item item); // Busco el item en la lista y lo borro
	void RemoveOnce(Item item); //Busco el item en la lista y lo borro en la primera
...
Sacamos el elemento item de la lista.
Funcion agregada por nosotros para implementar la solucion a la inversion de prioridades.
...
{
 ListNode *element, *prev_element;

    if (!IsEmpty()) {		// list not is empty
    
        //Nos fijamos si es el primer item el que queremos borrar de la lista
        if (item == first->item) {
            element = first;
            first = first->next;
            delete element;
            ...
            //Si no lo es, empezamos a buscar a partir del segundo elemento, donde:
            //element es el nodo auxiliar en que nos sirve para buscar el elemento deseado
            //prev_element es el nodo previo a element, nos sirve para mantener la coherencia de la lista en claso de que tengamos que borrar element
            
            prev_element = first;
            for (element = first->next; element != NULL; prev_element = element, element = element->next) {
            	DEBUG('b',"Pasada %d fp %d\n",element->item,item);
                if (element->item == item) {
                    //Si element es el elemento buscado, enlazamos el nodo previo a el con el siguiente a el y lo borramos,
                    //tambien nos fijamos si el que borramos es el ultimo, para actualizar el punteroa last
...
RemoveOnce(Item item)

Sacamos el elemento item de la lista (si esta varias veces, solo sacamos la primera que encontramos).

*/
template <class Item>
void
List<Item>::RemoveOnce(Item item)
{
 ListNode *element, *prev_element;
